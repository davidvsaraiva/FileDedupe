...
import org.pz.filededupe.outputinterfaces.OutputInterfacesModule;
...
public class Main {
    ...
    private static void outputDupes(LongStringListTable dupesTable, boolean tohtml) {
        Injector injector = Guice.createInjector(new OutputInterfacesModule());
        StandardOutput standardOutput = injector.getInstance(StandardOutput.class);
        // Scan the dupesTable and print out all duplicates to stdout and html
        standardOutput.output(dupesTable);
        if (tohtml) {
            HtmlOutput htmlOutput = injector.getInstance(HtmlOutput.class);
